...
public class tiny_gp {
  ...
  // traverse return the number of nodes in a program
  int traverse( char [] buffer, int buffercount ) {
    // if the current node contains a function less than ADD (110)
    if ( buffer[buffercount] < FSET_START )
        // increment the count by one and return that.
        return( ++buffercount );

    // if the current buffer contains a valid function
    switch(buffer[buffercount]) {
        // if the function is a two-branch function
        case AND: 
        case OR: 
            // return the result of the traversal of the current tree and the
            return( traverse( buffer, traverse( buffer, ++buffercount ) ) );
        // if the function is a single-branch function
        case NOT:
  ...
  int fitness_function( char [] Prog ) {
    int i = 0, len;
    int fit = 0;
    boolean result;
  ...
  int grow( char [] buffer, int pos, int max, int depth ) {
    ...
    public tiny_gp( String fname, long s ) {
        ...
        {
            // This change was not included in his instructions
            x[i]= rd.nextBoolean();
        }
